...
```{r}
fact_out <- factanal(sample_df[,-1], factors = 2, rotation = "varimax") 
...
ggplot(fact_out,aes(x=fl1,y=fl2,label=variable))+
  geom_segment(aes(xend=fl1,yend=fl2,x=0,y=0),arrow = arrow())+
  geom_text(color='red',nudge_y = -0.05)

```

```{r}
# 1) Build a tidy data frame for plotting
df_plot <- loadings_df %>%
  dplyr::transmute(
    variable = stock,
    industry = industry,
...
# 2) Plot arrows from origin to each loading pair
library(ggplot2)
...
```{r}
# Use your already computed industry_factor table:
industry_plot <- industry_factor %>%
  dplyr::rename(fl1 = mean_F1, fl2 = mean_F2)
