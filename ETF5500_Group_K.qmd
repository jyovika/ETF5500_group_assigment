...
cor(pc1_ts, market_df$MarketReturn)
```
## Question 1
```{r}
...
industry_factor %>%
  ...
  select(Industry, 
         ...
         `Mean F2` = mean_F2) %>%
  kbl(
    caption = "Mean Factor Loadings by Industry",
    digits = 3,
    align = c("l","r","r")
  ) %>%
  kable_styling(
...
industry_avg <- stocks_long %>%
  group_by(Date, industry_name) %>%
  summarise(AvgReturn = mean(ret, na.rm = TRUE), .groups = "drop") %>%
  pivot_wider(names_from = industry_name, values_from = AvgReturn)


M <- t(scale(industry_avg[,-1]))  


hc <- hclust(dist(M), method = "ward.D2")

plot(hc, main = "Hierarchical clustering of industries (Ward.D2)",
     xlab = "", sub = "", labels = rownames(M))
